...
module EdmentumClient
  class EdApiModelsClass
    ...
    def self_enroll_password=(self_enroll_password)
      if !self_enroll_password.nil? && self_enroll_password !~ Regexp.new(/^[a-zA-Z0-9@#$%&*+\\-_(),+':;?.,!\\[\\]\\s\\\\]{1,40}$/)
        fail ArgumentError, 'nil password sent'
      end
    ...
    def hash
      [program_id, resource_node_id, resource_version, resource_node_name, class_id, name, description, start_date, end_date, lock_assignment_on_due_date, allow_self_enroll, is_active, self_enroll_code, self_enroll_password, mastery_test_attempts].hash
    end
